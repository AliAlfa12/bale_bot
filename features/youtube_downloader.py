    ...
    try:
        ...
        for file in os.listdir('.'):
            if file.endswith(extensions):
                if safe_title in file or any(ext in file for ext in extensions):
                    return file
...
def download_youtube_audio(url, chat_id, send_message_func):
    send_message_func(chat_id, "🎵 در حال پردازش ویدیو برای استخراج صدا...")
    ...
    if youtube_cookies_b64:
        ...
        logger.warning("⚠️ YOUTUBE_COOKIES not found for audio.")
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if downloaded_file:
                ...
                logger.error("Downloaded audio file not found after download.")
                return None, "❌ فایل صوتی پیدا نشد"
    
    except Exception as e:
        error_msg = str(e)
        logger.error(f"YouTube audio download failed: {error_msg}")
        if "Sign in to confirm" in error_msg:
            return None, "❌ نیاز به احراز هویت. لطفاً کوکی YOUTUBE_COOKIES را به‌روز کنید."
        return None, f"❌ خطا: {error_msg[:100]}"
